...
class SVM:
    ...
    def solve_dual(self, x, y):
        ...
        def objective(alphas):
            ...
            gradient = y*np.dot(K,alphas*y)-1.
            return (obj, gradient)
    ...
    def compute_bias(self, x, y, alphas, support_vector_indices, kernel_func):
        ...
        if margin_vector_indices.shape[0]==0:
            ...
            negy_XOR_SV=np.logical_xor(negy,SV)
        ...
        #print (alphas.shape[0],ei_UB.shape,ei_LB.shape)
            ...
            #print ('support vectors on the margin found!')
            bias=ei_tot[margin_vector_indices[0]]
        return bias
    ...
    def _predict(self, inputs, support_multipliers, support_vectors,
        ...
        for sample in inputs:
            ...
            f.append(fi)
        f = np.array(f)
        predictions=np.array([1 if y>0 else -1 for y in f])
        return predictions
